...
public class HomeCommand extends BaseCommand {
    ...
    @CommandPermission("commandpack.command.home.other")
    @Subcommand("other")
    public void onDefault(Player player, OnlinePlayer onlinePlayer) {
        StringBuilder list = new StringBuilder("§aname §8- §adisplayName\n");
        this.plugin.getUserManager().getUser(onlinePlayer.getPlayer()).getHomes().forEach((k, n) -> {
            list.append(n.getName()).append(", ");
        });
        player.sendMessage(Prefixes.DEFAULT + Msgs.HOME_LIST.replace("{0}", list));
    }
    ...
    @Syntax("<player> <home>")
    @CommandPermission("commandpack.command.home.other")
    @CommandCompletion("@players @homes_other")
    @Subcommand("other")
    public void onHome(Player player, OnlinePlayer onlinePlayer, @Single String homeName) {
        UserHome home = this.plugin.getUserManager().getUser(onlinePlayer.getPlayer()).getHome(homeName);
        if (home == null) {
            player.sendMessage(Prefixes.DEFAULT + Msgs.HOME_NOTFOUND.replace("{0}", homeName));
            return;
        }
